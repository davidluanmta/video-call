...
io = new Server({
    ...
    cors: {
        origin: ['http://chat.tinducsoftware.com'],
    }
}).listen(server);
...
app.get(['/test'], (req, res) => {
    ...
    fetch(hostCfg.apiUrl + '/Account/Authentication?token=' + req.query.room)
        .then(res => res.json())
        .then(response => {
...
app.get('/join/', (req, res) => {
    if (hostCfg.authenticated && Object.keys(req.query).length > 0) {
        ...
        */
        const { room, name, audio, video, screen, notify, token } = req.query;
        // all the params are mandatory for the direct room join
        if (room && name && audio && video && screen && notify) {
            const result = fetch(hostCfg.apiUrl + '/Account/Authentication?token=' + req.query.room)
                .then(res => res.json())
                .then(response => {
                    if (response && response.status == 200) {
...
async function checkAuthen(token) {
    return new Promise((resolve, reject) => {
        request('http://localhost:7749/Account/CheckUser?id=' + token)
            .then(resolve)
            .catch((error) => {
            });
    });
